...
class TrendController extends Controller
{
    ...
    public function equipmentTrend(string $equipment, string $start_date = null, string $end_date = null)
    {
        ...
        if (in_array($equipment_code, $this->motorService->motorCodes()) && in_array($equipment, $this->motorService->registeredMotors())) {
            ...
            Log::info("trend motor $equipment [$start_date - $end_date] viewed by user", ['user' => session('user')]);
            ...
            Log::info("trend trafo $equipment [$start_date - $end_date] viewed by user", ['user' => session('user')]);
        ...
        if ($validator->passes()) {
            ...
            if ($validated['generate_pdf'] == 'true') {
                ...
                if (in_array($equipment_code, $this->motorService->motorCodes()) && in_array($equipment, $this->motorService->registeredMotors())) {
                    ...
                    Log::info("pdf trend motor $equipment [$start_date - $end_date] generated by user", ['user' => session('user')]);
                    ...
                    Log::info("pdf trend trafo $equipment [$start_date - $end_date] generated by user", ['user' => session('user')]);
                    return redirect()->action([PdfController::class, 'reportEquipmentPdf'], [
                        'type' => 'trafos',
                        'equipment' => $equipment,
                        'start_date' => $start_date,

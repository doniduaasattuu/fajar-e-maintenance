...
<form action="/{{ $action }}" method="post" id="forms">
    ...
    <div class="mb-3">
        ...
        @default
        <label for="{{ $column }}" class="form-label fw-semibold">{{ ucfirst(str_replace("_", " ", $column)) }}</label>
        <input @readonly($column=='updated_at' ||$column=='created_at' ) value="{{ ($column == 'created_at' || $column == 'updated_at') ? Carbon\Carbon::now() : null }}" value="{{ $motor->$column }}" id="{{ $column }}" name="{{ $column }}" type="text" maxlength="50" class="form-control">
        @endswitch
    ...
    <div class="mb-3">
        ...
        @case('weight')
        <label for="{{ $column }}" class="form-label fw-semibold">{{ ucfirst(str_replace("_", " ", $column)) }}</label>
        <input onkeypress="return onlynumber(event)" value="{{ $motor->$column }}" id="{{ $column }}" name="{{ $column }}" type="text" maxlength="10" class="form-control">
        @break

...
<form action="/{{ $action }}" method="post" id="forms">
    ...
    <!-- MOTOR DETAILS IS NOT SET -->
    @unless ($motorDetail)
    <div>
        @foreach ($motorService->getColumns('motor_details', ['id', 'created_at', 'updated_at']) as $column) <!-- FORM MOTOR DETAILS -->
        <div class="mb-3">
            @switch($column)

            @case('motor_detail')
            <input hidden readonly value="{{ $motor->id }}" id="{{ $column }}" name="{{ $column }}" type="text" class="form-control">
            @break
